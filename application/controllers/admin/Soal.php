...
class Soal extends CI_Controller{
	...
	function ajaxPaginationData(){
        ...
        $config['base_url']    = base_url().'admin/soal/ajaxPaginationData';
	...
	function cobaQuery($params = array()){
        ...
        $soal = $soal->where('soal_untuk',$this->kegiatan);

        $soal = $soal->order_by('soal_date','desc');
        //sort data by ascending or desceding order
        if(!empty($params['search']['sortBy'])){
           $soal = $soal->order_by('soal_date_update',$params['search']['sortBy']);
        }else{
           $soal = $soal->order_by('soal_date_update','desc');
        }
        ...
        if(!empty($params['search']['untukBy'])){
            $soal = $soal->where('soal_untuk',$params['search']['untukBy']);
        }
		...
		foreach ($soal->result_array() as $row){
            ...
            $baris['soal_date_update']     = $row['soal_date_update'];
    ...
    function ajaxPaginationData1(){
        ...
        $config['base_url']    = base_url().'soal/ajaxPaginationData1';
        $config['total_rows']  = $totalRec;
        $config['per_page']    = $this->perPage;
    ...
    function ajaxPaginationData2(){
        ...
        $config['base_url']    = base_url().'admin/soal/ajaxPaginationData2';
        $config['total_rows']  = $totalRec;
        $config['per_page']    = $this->perPage;
        $config['link_func']   = 'searchFilter';
    ...
    function ajaxPaginationDataTerkumpul(){
        ...
        $config['base_url']    = base_url().'admin/soal/ajaxPaginationDataTerkumpul';
        $config['total_rows']  = $totalRec;
        $config['per_page']    = $this->perPage;
        $config['link_func']   = 'searchFilter';
    ...
    function simpan_duplikat_soalby(){
        ...
        $users = $this->db->get_where('soal', array(
            'soal_pelajaran' => $pelajaran,
            'soal_guru' => $guru,
            'soal_kelas' => $kelas,
            'soal_untuk' => $untuk,
    ...
    function getpelajaranbykelas(){
        ...
        if( !empty($kelas) ){
            $this->db->where('soal_pembuat_kelas',$kelas);
        }
        if( !empty($untuk) ){
            $this->db->where('soal_pembuat_untuk',$untuk);
        }
        ...
        foreach($this->db->get()->result() as $row){
            ...
            array_push($items, $row->soal_pembuat_pelajaran);
    ...
    function getgurubykup(){
        ...
        foreach($this->db->get()->result() as $row){
            ...
            array_push($items, $row->soal_pembuat_guru);
    ...
    function getguru(){
        ...
        foreach($query->result() as $row){
            ...
            $data['label3'] = $row->soal_pembuat_jurusan;
        }
