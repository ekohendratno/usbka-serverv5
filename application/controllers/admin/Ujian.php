...
class Ujian extends CI_Controller{
    ...
    function cobaQuery2($params = array()){
        ...
        foreach ($ujian->result_array() as $row1){
            ...
            if(!empty($params['search']['sortBy'])){
                ...
                $ujian1 = $ujian1->order_by('ujian_mulai','asc');
            }
            ...
            $ujian1 = $ujian1->group_by('ujian_mulai');
            ...
            foreach ($ujian1->result_array() as $row2){
                ...
                $data_ujian1['ujian_jam'] = $row2['ujian_mulai'];
                $data_ujian1['jam'] =  array();

                $ujian2 = $this->db->select('*')->from('ujian');

                $ujian2 = $ujian2->where('ujian_tahunajaran',$this->tahunajaran);
                $ujian2 = $ujian2->where('ujian_untuk',$this->kegiatan);

                $ujian2 = $ujian2->where('ujian_tanggal',$row2['ujian_tanggal']);
                $ujian2 = $ujian2->where('ujian_mulai',$row2['ujian_mulai']);

                $ujian2 = $ujian2->get();
                foreach ($ujian2->result_array() as $row3){
                    $data_ujian2 = array();

                    $data_ujian2[ 'ujian_id' ] = $row3['ujian_id'];
                    $data_ujian2[ 'ujian_tanggal' ] = $row3['ujian_tanggal'];
                    $data_ujian2[ 'ujian_mulai' ] = date("H:i:s", strtotime($row3['ujian_mulai']));
                    $data_ujian2[ 'ujian_selesai' ] = date("H:i:s", strtotime('+'.$row3['ujian_waktu'].' minutes',strtotime($row3['ujian_mulai'])));
                    $data_ujian2[ 'ujian_mulai_indo' ] = $this->m->tanggalhari2( $row3['ujian_tanggal'],true );
                    $data_ujian2[ 'ujian_waktu' ] = $row3['ujian_waktu'];
                    $data_ujian2[ 'ujian_jenis' ] = $row3['ujian_jenis'];
                    $data_ujian2[ 'ujian_jumlah_soal' ] = $row3['ujian_jumlah_soal'];
                    $data_ujian2[ 'ujian_agama' ] = $row3['ujian_agama'];
                    $data_ujian2[ 'ujian_pelajaran' ] = $row3['ujian_pelajaran'];

                    $data_ujian2[ 'ujian_kelas' ] = $row3['ujian_kelas'];
                    $data_ujian2[ 'ujian_jurusan' ] = $row3['ujian_jurusan'];

                    $data_ujian2[ 'ujian_guru' ] = $row3['ujian_guru'];
                    $data_ujian2[ 'ujian_untuk' ] = $row3['ujian_untuk'];
                    $data_ujian2[ 'ujian_tahunajaran' ] = $row3['ujian_tahunajaran'];


                    array_push($data_ujian1['jam'], $data_ujian2);
                }
    ...
    function getpelajaranbykelas(){
        ...
        foreach($this->db->get()->result() as $row){
            ...
            array_push($items, $row->soal_pelajaran);
    ...
    function getgurubykup(){
        ...
        foreach($this->db->get()->result() as $row){
            ...
            array_push($items, $row->soal_guru);
    ...
    function koreksiby(){
        ...
        echo json_encode(array(
            "success"=> true,
            "pesan" => "<font color='green'><i class='fas fa-check'></i> Success dikoreksi !</font>"
        ));
    }
