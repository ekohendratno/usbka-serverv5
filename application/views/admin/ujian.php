...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $('[name="ujian_kelas"]').on('change', function() {
            var kelas = $('[name="ujian_kelas"]').val();
            var untuk = $('[name="ujian_untuk"]').val();
            $.ajax({
                type:'GET',
                data: 'kelas='+kelas+'&untuk='+untuk,
                url:'<?php echo base_url('admin/ujian/getpelajaranbykelas') ;?>',
                cache: false,
                dataType:'json',
                success: function(hasil){

                    $('[name="ujian_pelajaran"]').empty();
                    $('[name="ujian_guru"]').empty();
                    for(emp in hasil){
                        $('[name="ujian_pelajaran"]').append('<option value="'+hasil[emp]+'">'+hasil[emp]+'</option>');
                    }
        ...
        $('[name="ujian_pelajaran"]').on('change', function() {
            ...
            $.ajax({
                type:'GET',
                data: 'kelas='+kelas+'&untuk='+untuk+'&pelajaran='+pelajaran,
                url:'<?php echo base_url('admin/ujian/getgurubykup') ;?>',
                cache: false,
                dataType:'json',
                success: function(hasil){
                    ...
                    $('[name="ujian_guru"]').empty();
                    for(emp in hasil){
                        $('[name="ujian_guru"]').append('<option value="'+hasil[emp]+'">'+hasil[emp]+'</option>');
                    }
        ...
        if(data.length < 1 || !data){
            ...
            for(emp2 in data){
                ...
                var empRow = '<div class="list-group-item list-group-item-warning">'+
                    '<p class="list-group-item-text title" style="text-align:left;"><span class="fas fa-calendar"></span> '+data[emp2].ujian_tanggal_indo+'</p>'+
                    '<div class="clearfix"></div>'+
                    '</div>';
                $('#postList0').append(empRow);

                for(emp1 in tanggal) {
                    var jam = tanggal[emp1].jam;

                    var empRow = '<div class="list-group-item list-group-item-info">'+
                        '<p class="list-group-item-text title" style="text-align:left;"><span class="fas fa-clock"></span> '+tanggal[emp1].ujian_jam+'</p>'+
                        '<div class="clearfix"></div>'+
                        '</div>';
                    $('#postList0').append(empRow);
                    ...
                    for(emp in jam) {
                        ...
                        var empRow = '<div class="list-group-item">'+
                            '<p class="list-group-item-text title" style="text-align:left;">'+
                            ' <span class="label label-default">'+jam[emp].ujian_mulai+'</span>'+
                            ' <span class="label label-default">'+jam[emp].ujian_selesai+'</span>'+
                            ' <span class="label label-default">'+jam[emp].ujian_untuk+'</span>'+
                            ' <span class="label label-default">'+jam[emp].ujian_kelas+'</span>'+
                            ' <span class="label label-default">'+jam[emp].ujian_jurusan+'</span>'+
                            ' <span class="label label-default">'+jam[emp].ujian_agama+'</span>'+
                            '</p><br/>'+
                            '<div class="col-md-8"><div class="row">'+
                            '<p><i style="color:#999">'+jam[emp].ujian_guru+'</i></p>'+
                            '<h3 class="list-group-item-heading name"><a data-backdrop="static" data-keyboard="false" href="#formtampil" data-toggle="modal" onClick="submitPreviewSoal(\''+jam[emp].ujian_tahunajaran+'\',\''+jam[emp].ujian_pelajaran+'\',\''+jam[emp].ujian_guru+'\',\''+jam[emp].ujian_kelas+'\',\''+jam[emp].ujian_untuk+'\')">'+jam[emp].ujian_pelajaran+'</a></h3>'+
                            '<p><i style="color:#999">Waktu '+jam[emp].ujian_waktu+' menit</i>, <i style="color:#999">Jenis ujian '+jam[emp].ujian_jenis+'</i>, <i style="color:#999">'+jam[emp].ujian_jumlah_soal+' buah soal</i></p>'+
                            '</div></div>'+
                            '<div class="col-md-4" style="text-align:left;"><div class="row">'+

                            '<a title="Preview" title="Preview" data-backdrop="static" data-keyboard="false" href="#formDialogHasil" data-toggle="modal" onClick="submitHasil('+jam[emp].ujian_id+')" class="btn btn-circle btn-default" style="color: #4c4c4c;" ><span class="fas fa-eye"></span></a>'+
                            '<a title="Export Data" title="Duplikat" onClick="submitExport('+jam[emp].ujian_id+')" class="btn btn-circle btn-default" style="color: #6db571;" ><span class="fas fa-file-export"></span></a>'+
                            '<a title="Duplikat Data" title="Duplikat" onClick="submitDuplikat('+jam[emp].ujian_id+')" class="btn btn-circle btn-default" style="color: #4c4c4c;" ><span class="fas fa-clone"></span></a>'+
                            '<a title="Ubah Data" title="Ubah" data-backdrop="static" data-keyboard="false" href="#formDialog" data-toggle="modal" onClick="formDialog('+jam[emp].ujian_id+')" class="btn btn-circle btn-default" style="color: #5cb85c;" ><span class="fas fa-pen"></span></a>'+
                            '<a title="Hapus" onclick="submitHapus('+jam[emp].ujian_id+')" class="btn btn-circle btn-default" style="color: #d9534f;"><span class="fas fa-trash"></span></a>'+
    ...
    function submitExport(x) {
        window.open("<?php echo base_url();?>export/ujian?id="+x,"_self");
    }
        ...
        if(data.length < 1 || !data){
            ...
            for(emp in data){
                ...
                var empRow = '<div class="list-group-item">'+
                    ...
                    '<a title="Koreksi" title="Koreksi" onclick="submitKoreksi('+data[emp].soal_jawab_id+')" class="btn btn-circle btn-default"><span class="fas fa-atom"></span></a>'+
    ...
    function submitKoreksi(id) {
        var tanya = confirm('Apakah yakin mau koreksi data?');
        if(tanya){
            $.ajax({
                type:'GET',
                data: 'id='+id,
                url:'<?php echo base_url('admin/ujian/koreksiby') ;?>',
                cache: false,
                dataType:'json',
                success: function(hasil){
                    submitHasil(id);
